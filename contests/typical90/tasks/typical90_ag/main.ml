...
let[@warning "-32"] build_directed_graph n edges =
...
let[@warning "-32"] build_undirected_graph n edges =
  let g = Array.create ~len:n [] in
...
let solve h w =
  ...
  then h * w
    ...
    let open Z in
    of_int h /> of_int 2 * (of_int w /> of_int 2) |> to_int
;;

let () =
  let h, w = sscanf (line ()) " %d %d" Tuple2.create in
  let ans = solve h w in
  printf "%d\n" ans
;;
