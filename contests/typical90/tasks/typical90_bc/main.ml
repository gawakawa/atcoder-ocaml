...
(** {[
      let module Mint = Modint (struct
          let m = p
        end)
      in
      Mint.((a * b) + c)
    ]} *)
module [@warning "-32"] Modint (M : sig
    val m : int
  end) =
struct
  let ( + ) a b = (a + b) % M.m
...
let q_mod_p p q n_lst =
  let module Mint = Modint (struct
      let m = p
    end)
  in
  Mint.(
    let product = List.fold_left ~init:1 ~f:( * ) in
    product n_lst = q)
;;

let solve p q lst = Iter.filter_count (q_mod_p p q) (combinations 5 lst)

let () =
  let _n, p, q = sscanf (line ()) " %d %d %d" Tuple3.create in
  let a_lst = read_int_lst () in
  let ans = solve p q a_lst in
  printf "%d\n" ans
;;
