...
let solve lst =
  let original = List.map ~f:String.lowercase lst in
  let deduped = List.stable_dedup original ~compare:String.compare in
  List.map deduped ~f:(fun s -> List.count original ~f:(String.equal s))
