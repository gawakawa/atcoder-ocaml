...
let closer pos ma mb =
  let cmp a b = compare (abs (a - pos)) (abs (b - pos)) in
  match ma, mb with
  | None, None -> assert false
  | Some a, None -> a
  | None, Some b -> b
  | Some a, Some b when Comparable.equal cmp a b -> min a b
  | Some a, Some b -> Comparable.min cmp a b
;;
...
let solve lst =
  ...
  let _, _, sum_d =
    Fn.apply_n_times
      ~n
      (fun (st, curr, acc) ->
         let max_less = Set.binary_search st ~compare `Last_strictly_less_than curr in
         let min_greater =
           Set.binary_search st ~compare `First_strictly_greater_than curr
         in
         let closest = closer curr max_less min_greater in
         let new_st = Set.remove st closest in
         new_st, closest, acc + abs (closest - curr))
      (Set.of_list (module Int) lst, 0, 0)
  in
  sum_d
;;
