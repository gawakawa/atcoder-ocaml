...
let solve p_lst q_lst =
  ...
  let p_arr = Array.of_list p_lst in
  let q_arr = Array.of_list q_lst in
  let perms = permutations (Array.init n ~f:succ) in
  List.count perms ~f:(fun perm ->
    compare_array compare perm p_arr > 0 && compare_array compare perm q_arr < 0)
;;

let () =
  let _n = read_int_opt () |> Option.value_exn in
  let p_lst = read_int_lst () in
  let q_lst = read_int_lst () in
  let ans = solve p_lst q_lst in
  printf "%d\n" ans
;;
